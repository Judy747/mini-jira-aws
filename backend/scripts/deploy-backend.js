...
function buildDeployScript(branch) {
  return [
    ...
    'if [ -f "$APP_DIR/backend/scripts/refresh-ec2-env-from-ssm.sh" ]; then',
    '  bash "$APP_DIR/backend/scripts/refresh-ec2-env-from-ssm.sh"',
    'else',
    '  SNS_ARN=$(aws ssm get-parameter --region "${AWS_REGION:-us-east-1}" --name /mini-jira/prod/SNS_TASK_ASSIGNMENT_TOPIC_ARN --query Parameter.Value --output text 2>/dev/null || true)',
    '  if [ -n "$SNS_ARN" ] && [ "$SNS_ARN" != "None" ]; then',
    '    touch /etc/mini-jira.env; chmod 600 /etc/mini-jira.env',
    '    grep -v "^SNS_TASK_ASSIGNMENT_TOPIC_ARN=" /etc/mini-jira.env > /tmp/mini-jira.env.$$ || true',
    '    echo "SNS_TASK_ASSIGNMENT_TOPIC_ARN=$SNS_ARN" >> /tmp/mini-jira.env.$$',
    '    mv /tmp/mini-jira.env.$$ /etc/mini-jira.env',
    '    chown miniapp:miniapp /etc/mini-jira.env',
    '    echo "[deploy] set SNS_TASK_ASSIGNMENT_TOPIC_ARN from SSM"',
    '  else',
    '    echo "[deploy] WARN: could not load SNS_TASK_ASSIGNMENT_TOPIC_ARN from SSM"',
    '  fi',
    'fi',
